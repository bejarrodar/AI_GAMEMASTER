...
if ! grep -qi "amzn\\|amazon linux" /etc/os-release; then
  echo "This installer is for Amazon Linux. Detected:"
  cat /etc/os-release
  exit 1
fi
...
After=network.target postgresql.service postgresql-15.service ollama.service
